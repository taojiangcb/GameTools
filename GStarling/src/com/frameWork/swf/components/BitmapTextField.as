...
package com.frameWork.swf.components
{
	 ...
	 * 
	 *  <p>这个是TextField的简化版本,只支持纹理文本，且使用QuadBatch渲染</p>
	 */
	public class BitmapTextField extends SwfQuadBatch
	{
		// the name container with the registered bitmap fonts
		private static const BITMAP_FONT_DATA_NAME:String = "com.gameabc.sanguo.components.BitmapCoustomFont";
		...
		private var mIsCoustomColor:Boolean			= false;
		private var mHitArea:Rectangle;
		private var mBorder:SwfQuadBatch;
		private var mQuadBatch:SwfQuadBatch;
		...
		public function BitmapTextField(width:int, height:int, text:String, fontName:String="Verdana",
		...
		private function createComposedContents():void
		{
			if (mQuadBatch == null) 
			{ 
				mQuadBatch = new SwfQuadBatch(); 
				mQuadBatch.touchable = false;
			}
			else
				mQuadBatch.reset();
			
			var bitmapFont:BitmapCoustomFont = getBitmapFont(mFontName);
			...
			bitmapFont.fillQuadBatch(mQuadBatch,
				width, height, mText, mFontSize, 
				mColor, hAlign, vAlign, mAutoScale, 
				mKerning,mIsCoustomColor);
		...
		public function set fontName(value:String):void
		{
			if (mFontName != value)
			{
				if (value == BitmapCoustomFont.MINI && bitmapFonts[value] == undefined)
					registerBitmapFont(new BitmapCoustomFont());
		...
		public override function get batchable():Boolean { return super.batchable; }
		public override function set batchable(value:Boolean):void
		{ 
		...
		public static function registerBitmapFont(bitmapFont:BitmapCoustomFont, name:String=null):String
		{
		...
		public static function getBitmapFont(name:String):BitmapCoustomFont
		{
		 ...
		 * @param bol
		 */		
		public function set isCoustomColor(bol:Boolean):void
		{
			if(bol == mIsCoustomColor) return;
			mIsCoustomColor = bol;
			if(mIsCoustomColor) mRequiresRedraw = true;
		}
		
		public function get isCoustomColor():Boolean
		{
			return mIsCoustomColor;
		}
